...
func (bc *bceCDNStorageMiddleware) URLFor(ctx context.Context, path string, options map[string]interface{}) (string, error) {
	if !(bc.StorageDriver.Name() != "s3" || bc.StorageDriver.Name() != "bos") {
		dcontext.GetLogger(ctx).Warn("the BceCDN middleware does not support this backend storage driver")
...
func init() {
	storagemiddleware.Register("bcecdn", newBceCDNStorageMiddleware)
}